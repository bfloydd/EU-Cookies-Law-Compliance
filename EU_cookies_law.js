...
function EU_cookies_law(r) {
	...
	if (false !== navigator.cookieEnabled) {
		...
		var getCookie = function getCookie(sName) {
			...
			if (oRegex.test(document.cookie)) return decodeURIComponent(RegExp.$1);else return null;
		};

		// Check current website cookie if exists
		var check_cookies = function check_cookies() {
		...
		// Cookies creation
		var cookie_creation = function cookie_creation(c, e) {
		...
		var jsloader = function jsloader(el) {
			// Arrays
			var s = [],
